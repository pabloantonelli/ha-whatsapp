...
import {
  ...
  DisconnectedError,
  typingDelayMs,
} from "../src/baileys-client.js";

const client = new BaileysClient({ path: "/tmp/does-not-matter" });
...
describe("errores", () => {
  it("conserva el mensaje original en vez de descartarlo", () => {
    const err = new MessagingError(428, new Error("socket hung up"));
  ...
  it("etiqueta los códigos desconocidos sin romperse", () => {
    expect(new MessagingError(999).message).toContain("Unknown Error");
  });

  it("marca la desconexión con código 401", () => {
    expect(new DisconnectedError().code).toBe(401);
  });
});
